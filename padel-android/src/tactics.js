(() => {
  ...
  class RallyTactics {
    ...
    reset(){this.brains=Array.from({length:4},(_,id)=>({state:'Recover',style:styles[id],history:[],fatigue:0,next:0,shot:-1,readyAt:0,perceiveAt:0,plan:null}));this.rallyStart=null;this.history=[];this.safeguardCount=0;}
    ...
    choose(id){
      ...
      const netPressure=opponents.every(o=>Math.abs(o.z)<4.4),canAttack=b.y>2.25&&Math.abs(p.z)<5.5&&!defending;
      const candidates=[];
      for(const x of [-3.65,0,3.65])for(const depth of [2.8,5.4,8.3])for(const kind of ['drive','lob',...(canAttack?['smash']:[])]){
        ...
        score+=kind==='lob'?(netPressure?3.5:-1.8)+(defending?1.3:0)-recent.filter(h=>h.kind==='lob').length*2.1:0;
        score+=kind==='smash'?2.2:0;score+=kind==='drive'&&zone==='short'&&netPressure?-2.5:0;score+=kind==='drive'&&zone==='middle'&&Math.abs(x)<.1&&opponents.length===2?.85:0;score+=defending&&zone==='deep'?1.2:0;
      ...
      power=clamp(power+(pick.kind==='lob'?0:this.pressure()*.12)+clamp(g.gaussian(),-2,2)*error,.04,.99);
      return {...pick,power,intent,error,displaced};
    }
    recoveryTarget(id,pick){
      const g=this.game,team=g.team(id),ids=g.members(team),side=team===0?1:-1,brain=this.brains[id],incoming=g.lastHitter!==team;
      const lobThreat=incoming&&(g.ball.y>2.8||g.ball.vy>3.5),defend=lobThreat||brain.style==='defensive';
      const depth=incoming?(lobThreat?7.4:5.6):defend?6.1:g.lastShotSpeed>10||g.solvedKind==='lob'?3.25:4.8;
      // Partners cover separate lanes, moving together toward net or defensive positions.
      const center=clamp(g.ball.x*.35,-1.5,1.5),lane=ids.length===2?(pick?(pick.x>=0?-1:1):(ids.indexOf(id)===0?1:-1)):0;
      return {x:clamp(center+lane*2.1,-4.1,4.1),z:side*depth};
    }
    coordinate(){
      ...
      for(let team=0;team<2;team++){
        ...
        for(const id of ids){if(g.isHuman(id))continue;const brain=this.brains[id],p=g.players[id];
          if(incoming&&brain.shot!==g.rallyHits){brain.shot=g.rallyHits;brain.perceiveAt=g.time+settings.reaction*(1+brain.fatigue*.5);brain.readyAt=brain.perceiveAt;g.targets[id]={x:p.x,z:p.z};p.splitStart=g.time;brain.plan=null;brain.offset=g.gaussian()*settings.error;brain.next=0;this.transition(brain,'Track');}
          if(incoming&&g.time<brain.perceiveAt)continue;
          if(g.time<brain.next)continue;brain.next=g.time+levels.decision+settings.reaction*.3;
          if(incoming&&pick&&pick.id===id){
            const target={x:clamp(pick.x+(brain.offset||0),-4.5,4.5),z:side*clamp(Math.abs(pick.z)+Math.abs(brain.offset||0)*.3,.65,9.4)};
            if(Math.hypot(target.x-g.targets[id].x,target.z-g.targets[id].z)>.16)g.targets[id]=target;
            ...
            g.targets[id]=this.recoveryTarget(id,pick);
    ...
    hit(id){const g=this.game,brain=this.brains[id],team=g.team(id);if(g.claims[team]!==id||!brain.plan||g.time<brain.readyAt||g.time<(brain.cooldown||0)||!g.canHit(id)||g.serveActive&&(g.bounces[team]===0||id!==g.serveReceiver))return;
      const plan=this.choose(id),p=g.players[id],distance=Math.hypot(g.ball.x-p.x,g.ball.z-p.z),strain=clamp(distance-.55,0,1)+brain.fatigue*.3+this.pressure()*.12;
