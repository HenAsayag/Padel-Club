...
function gestureView(x,y){const r=renderer.domElement.getBoundingClientRect();if(isLocalCoop()&&(experience.coopView==='split'||cameraMode===4)){const left=x<r.left+r.width/2;return {id:left?0:2,camera:coopCameras[left?0:1],left:r.left+(left?0:r.width/2),top:r.top,width:r.width/2,height:r.height};}return {id:game.controlled,camera,left:r.left,top:r.top,width:r.width,height:r.height};}
...
function updateGestureUI(dt){
  ...
  $('gesture-help').hidden=game.mode!=='ready';$('gesture-help').textContent=game.mode==='ready'?(game.movementModes?.[game.controlled]==='joystick'?'Joystick to move · swipe to serve':'Swipe to serve · running is automatic'):'Auto-run · choose DRIVE or LOB, then swipe';
