...
function updateGestureUI(dt){
  ...
  if(shot){$('gesture-power-text').textContent=shot.invalid?'Swipe up to hit':(shot.kind==='lob'?'LOB':shot.power<.48?'GENTLE':shot.power>.82?'FIRM':'DRIVE');$('gesture-power-bar').value=shot.power||0;
