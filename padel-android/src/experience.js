...
function updateEyeCamera(view,p,b,side,id,dt){
  ...
  // Contact changes drop to rally before the low ball has left the server's feet.
  // Keep the court framing through departure, then blend into tracking in space.
  const ownServe=game.mode==='rally'&&game.serveActive&&game.serverPlayer===id;
  setEyePose(view.position,eyeGazeTarget,p,b,side,follow&&!ownServe);
  if(ownServe){const distance=Math.hypot(b.x-p.x,b.z-p.z);eyeGazeTarget.lerp(b,THREE.MathUtils.smoothstep(distance,2.8,5));}
