...
function animateClubCharacter(model,p,id,dt){
 if(dt<=0)return;const menu=game.mode==='menu',between=game.mode==='between'||game.mode==='match';if(!menu&&!between){const charge=game.chargeStates?.[id]||(swipe?.view.id===id?{power:swipe.last?.power??.2}:null);if(charge&&model.clock-model.contactAt>.3){const amount=Math.min(1,charge.power*2);model.torso.rotation.y-=amount*.18;model.hips.position.y-=amount*.025;model.limbs.right.arm.rotation.x+=amount*.25;model.limbs.right.forearm.rotation.x+=amount*.18;}return;}
