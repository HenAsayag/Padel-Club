...
async function enterGameScreen(force=false){
  if((!isPhone&&force!==true)||screenBusy||browserPlay)return;screenBusy=true;
  try{
    if(!document.fullscreenElement&&document.documentElement.requestFullscreen)await document.documentElement.requestFullscreen();
    if(isPhone&&screen.orientation?.lock)await screen.orientation.lock('landscape');
...
function refreshGameScreen(){
  ...
  $('menu-fullscreen').textContent=document.fullscreenElement?'FULL SCREEN ✓':'FULL SCREEN ↗';
...
function requestGameFullscreen(){setBrowserPlay(false);return enterGameScreen(true);}
$('menu-fullscreen').onclick=requestGameFullscreen;$('browser-fullscreen').onclick=requestGameFullscreen;$('fullscreen-play').onclick=requestGameFullscreen;$('rotate-fullscreen').onclick=requestGameFullscreen;
